...
export class AuthService {
  constructor(private prisma: PrismaService, private jwt: JwtService, private congig: ConfigService) { }
  ...
  async signToken(userId: number, email: string): Promise<{ access_token: string }> {
    ...
    const scretToken = await this.jwt.signAsync(data, { expiresIn: '2h', secret: secret });
    return {
